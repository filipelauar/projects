...
class NN(nn.Module):
    def __init__(self, inSize, outSize, layers=[], finalActivation=None, activation=torch.tanh,dropout=0.0, normalisation = False,final_normalisation = None, init_w = None):
        super(NN, self).__init__()
        self.normalisation = normalisation
        self.final_normalisation = final_normalisation
        self.layers = nn.ModuleList([])

        if normalisation:
            self.normalisations = nn.ModuleList([])
        for x in layers:
            if normalisation and len(self.layers) != 0:
                self.normalisations.append(torch.nn.BatchNorm1d(inSize,track_running_stats = True))
            self.layers.append(nn.Linear(inSize, x))
            inSize = x
        if normalisation:
            self.normalisations.append(torch.nn.BatchNorm1d(inSize,track_running_stats = True))
        ...
        if init_w is not None:
            layer = self.layers[-1]
            layer.weight.data.uniform_(init_w[0],init_w[1])
            layer.bias.data.uniform_(init_w[0],init_w[1])
    ...
    def forward(self, x):
        ...
        for i in range(1, len(self.layers)):
            ...
            if self.normalisation:
                x = self.normalisations[i - 1](x)
            x = self.layers[i](x)

        if self.finalActivation is not None:
            x=self.finalActivation(x)
            if self.final_normalisation is not None:
                x = self.final_normalisation(x)
