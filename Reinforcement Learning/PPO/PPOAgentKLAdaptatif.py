...
class PPOAgent_1(object):

    def __init__(self, env, opt, discount = 0.99,\
                 learning_rate = 1e-4, freq_opti = 10000,\
                sample_size = 10,lbda = 1, K = 1,KLC = 1, entropy = 0, reverse = False):
        """
        ...
        self.delta = opt.get("KLC",KLC)
        self.entropy = opt.get("entropy",entropy)
        self.reverse = opt.get("reverse",False)
        ...
        self.dkl = torch.nn.KLDivLoss(log_target = True)
    ...
    def learn(self):
        ...
        for _ in range(self.K):
            ...
            L = torch.mean((new_p[range_traj,actions]/old_p[range_traj,actions])*A)
            
            DKL =  self.dkl(new_p.log(),old_p.log()) if not self.reverse else self.dkl(old_p.log(),new_p.log())

            l_politique = -L + self.B*DKL
            if self.entropy > 0:
        ...
        DKL = self.dkl(self.politique(obs).log(),old_p.log()).detach() if not self.reverse else self.dkl(old_p.log(),self.politique(obs).log()).detach()

        if DKL >= 1.5*self.delta:
            self.B*=2
        if DKL <= self.delta/1.5:
            self.B*=0.5
        self.optimizer_critique.zero_grad()

        # new_A = self.get_new_cummulated_rewards()
        for i in range(nb_traj):
            #Apprentissage critique
            #taille_traj = len(self.replay[i])
            traj = np.array(self.replay[i])
            # A = torch.tensor(self.advs_replay[i])
            obs     = torch.Tensor(np.concatenate(traj[:,0])).to(self.device)
            # new_obs = torch.Tensor(np.concatenate(traj[:,3]))

            #On optimise le critique V
            y = self.cumulated_rewards[i].detach() # .reshape(-1,1) + self.gamma * self.critique_target(new_obs)
            ...
            l_critique.backward()
            with torch.no_grad():
                l_critique_tot += l_critique/len(traj)
        ...
        self.nbEventsLastUpdate = 0
        return float(l_politique_tot)/self.K, float(l_critique_tot)/ nb_traj, torch.mean(A), self.B, DKL
        ...
        with torch.no_grad():
            for traj in self.replay[:-1]: # for each trajectory
                ...
                for t in reversed(range(len(traj))):
                    ...
                    current = traj[t][0].to(self.device)
                    next = traj[t][3].to(self.device)
                    Vcur = self.critique_target(torch.tensor(current,dtype=torch.float))[0][0]
        ...
        if not self.test:
            ...
            if done or it == self.opt.maxLengthTrain:  # if end
                ...
                with torch.no_grad(): 
                    ...
                    cumulated_reward = torch.zeros(len(last_traj)).to(self.device) # depend of TD0, TD1 or TDlambda
                    
                    for t in reversed(range(len(last_traj))):
                        next_state = torch.tensor(last_traj[t][3][0],dtype=torch.float).to(self.device)
                        cur_state = torch.tensor(last_traj[t][0][0],dtype=torch.float).to(self.device)
